            ...
            if rank != "R#":
                ...
                for j in range(0, len(td)):
                    if td[j].find("span") != None and td[j].find("input") == None:
                        if k < 10:     # so that we don't pull total, we'll calculate that ourselves
                            formatted_data[k] = td[j].find("span").string.strip()
                            k += 1
...
def push_to_db(player_list, conn):
    insert_sql = """INSERT INTO public.hashtag_rankings_zscore (name, fg_percent, ft_percent, threepm, pts, treb, ast, stl, blk, turnovers) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"""
    delete_sql = """DELETE FROM public.hashtag_rankings_zscore"""
    cur = conn.cursor()
...
if __name__ == '__main__':
    ...
    logging.info('Scraped z-score data from hashtagbasketball.com')
